...
public class CatmullRomSpline : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        else
        {
            ...
            SamplePointsTemp = GeneratePointByCutMullRomWithDistance(controlPoints, 1f, false);
            SamplePoints = new Vector3[SamplePointsTemp.Count];

            for (int i = 0; i < SamplePointsTemp.Count; i++)
            {
                SamplePoints[i] = SamplePointsTemp[i];
            }
        }
    }
    ...
    private void OnDrawGizmos()
    {
        ...
        for (int i = 0; i < SamplePointsTemp.Count; i++)
        {
            Gizmos.DrawSphere(transform.TransformPoint(SamplePointsTemp[i]), 0.1f);
        }
    }
